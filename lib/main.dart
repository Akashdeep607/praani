...
import 'services/offline_mode/connectivity_controller.dart';
...
void _setupApplication() {
  ...
  injector.get<FirebaseMessagingService>().onBackgroundMessage(_firebaseMessagingBackgroundHandler);
}
...
void main() {
  ...
  runZonedGuarded(() async {
    ...
    Services().setAppConfig(serverConfig, ignoreInitCart: (Configurations.multiSiteConfigs?.isNotEmpty ?? false));
    ...
    if (serverConfig['type'] == 'vendorAdmin') {
      return runApp(Services().getVendorAdminApp(languageCode: languageCode, isFromMV: false) ?? const SizedBox());
    }

    if (serverConfig['type'] == 'delivery') {
      return runApp(Services().getDeliveryApp(languageCode: languageCode, isFromMV: false) ?? const SizedBox());
    }

    ResponsiveSizingConfig.instance
        .setCustomBreakpoints(const ScreenBreakpoints(desktop: 1000, tablet: 600, watch: 100));
