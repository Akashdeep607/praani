...
class UserModel with ChangeNotifier {
  ...
  Future<void> loginFirebaseSMS(
      {String? phoneNumber, required Function success, Function? fail, required BuildContext context}) async {
    try {
      print('LOGIN SMS - FIREBSESMS - user_model.dart');
  ...
  Future<void> loginGoogle({Function? success, Function? fail, context}) async {
    try {
      var googleSignIn = GoogleSignIn(scopes: ['email', 'profile'], serverClientId: '');
    ...
    try {
      ...
      if (user == null) {
        ...
        final hasInfoUser = cookies.any((element) =>
            ['pro_loyalty_api_session', 'customer_sig'].contains(element.name) && element.value.isNotEmpty);
  ...
  Future<void> _saveUser(User? user, {String? username, String? password}) async {
    try {
      ...
      if ((kAdvanceConfig.enableWooCommerceWholesalePrices || kAdvanceConfig.b2bKingConfig.enabled) &&
  ...
  Future<void> createUser({
    ...
    try {
      loading = true;
      notifyListeners();
      final isValidAccount = (email?.isEmail ?? false) && (password?.isNotEmpty ?? false);

      Future<void> loginAccountFirebase() async =>
          Services().firebase.createUserWithEmailAndPassword(email: email, password: password);
  ...
  Future<void> login({
    ...
    try {
      ...
      await _saveUser(user, username: username, password: password);
      success(user!);
      loading = false;
      notifyListeners();
    } catch (err) {
  ...
  Future<String?> getJwtAuth(String username, String password) async {
    try {
      var endPoint =
          'https://agratix.com/psdemo/wp-json/jwt-auth/v1/token?username=$username&password=$password'.toUri();
      var response = await http.post(endPoint!);
      var jsonDecode = convert.jsonDecode(response.body);
      if (jsonDecode['token'] == null) {
        throw Exception(jsonDecode['code']);
      }
      printError("[getJwtAuth]: ${jsonDecode['token']}");
      return jsonDecode['token'];
    } catch (e, trace) {
      printError('ERROR CATCH [getJwtAuth] fail: $trace');
