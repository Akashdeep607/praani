...
mixin PaymentMixin<T extends StatefulWidget> on State<T>, RazorDelegate {
  ...
  PaymentMethodModel get _paymentMethodModel => context.read<PaymentMethodModel>();
  ...
  void handlePaymentFailure(PaymentFailureResponse response) {
    ...
    if (body['error'] != null && body['error']['reason'] != 'payment_cancelled') {
      Tools.showSnackBar(ScaffoldMessenger.of(context), body['error']['description']);
    }
  ...
  void showSnackbar() {
    Tools.showSnackBar(ScaffoldMessenger.of(context), S.of(context).orderStatusProcessing);
  }
  ...
  Future<void> createOrder(
      {paid = false, bacs = false, cod = false, AdditionalPaymentInfo? additionalPaymentInfo}) async {
  ...
  void placeOrder(PaymentMethodModel paymentMethodModel, CartModel cartModel) {
    ...
    if (paymentMethodModel.paymentMethods.isNotEmpty) {
      final paymentMethod =
          paymentMethodModel.paymentMethods.firstWhere((item) => item.id == cartModel.paymentMethod?.id);

      var isSubscriptionProduct = cartModel.item.values.firstWhere(
              (element) => element?.type == 'variable-subscription' || element?.type == 'subscription',
              orElse: () => null) !=
          null;
      ...
      if (!isSubscriptionProduct && kBankTransferConfig.getValueList('paymentMethodIds').contains(paymentMethod.id)) {
        onLoading?.call(false);
        isPaying = false;
                      ...
                      if (order != null) {
                        for (var item in order.lineItems) {
                          ...
                          if (product?.bookingInfo != null) {
                            ...
                            Tools.showSnackBar(
                                ScaffoldMessenger.of(context), booking ? 'Booking success!' : 'Booking error!');
                          }
                        }
                        onFinish!(order);
                      }
      ...
      /// MercadoPago payment
      final availableMercadoPago = kMercadoPagoConfig.getValueList('paymentMethodIds').contains(paymentMethod.id) &&
      ...
      final availableRazorpay = kRazorpayConfig.getValueList('paymentMethodIds').contains(paymentMethod.id) &&
              ...
              if (order != null) {
                ...
                final razorpayServices = RazorServices(
                  ...
                  userInfo: RazorUserInfo(
                    ...
                    fullName: '${cartModel.address?.firstName ?? ''} ${cartModel.address?.lastName ?? ''}'.trim(),
                  ),
                  delegate: this,
                  onLoading: onLoading,
                );
      ...
      final availablePayTm =
          kPayTmConfig.getValueList('paymentMethodIds').contains(paymentMethod.id) && kPayTmConfig['enabled'] == true;
      ...
      /// PayStack payment.
      final availablePayStack = kPayStackConfig.getValueList('paymentMethodIds').contains(paymentMethod.id) &&
          kPayStackConfig['enabled'] == true;
      if (!isSubscriptionProduct && availablePayStack) {
        final isSupported = List.from(kPayStackConfig['supportedCurrencies'] ?? [])
                .firstWhereOrNull((e) => e.toString().toLowerCase() == cartModel.currencyCode?.toLowerCase()) !=
            null;
        if (isSupported) {
          createOrderOnWebsite(
              paid: false,
          ...
          Tools.showSnackBar(ScaffoldMessenger.of(context),
              S.of(context).currencyIsNotSupported(cartModel.currencyCode?.toUpperCase() ?? ''));
        }
        return;
      }

      /// Flutterwave payment.
      final availableFlutterwave = kFlutterwaveConfig.getValueList('paymentMethodIds').contains(paymentMethod.id) &&
      ...
      /// PhonePe payment.
      final availablePhonePe = kPhonePeConfig.getValueList<String>('paymentMethodIds').anyTheSame(paymentMethod.id) &&
        ...
        success: (Order? order) async {
          ...
          if (order != null) {
            ...
            for (var key in cartModel.productsInCart.keys) {
              var productBooking = cartModel.cartItemMetaDataInCart[key]?.bookingInfo;
              if (productBooking != null) {
                productBooking.idOrder = order.id;
                booking = await createBooking(productBooking)!;
                Tools.showSnackBar(ScaffoldMessenger.of(context), booking ? 'Booking success!' : 'Booking error!');
              }
              // }
            }
            onFinish!(order);
